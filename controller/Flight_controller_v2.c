...
// - go to t-crest/patmos/c/apps/de10-=nano folder
// - make
...
// - wait for the IMU to callibrate which is indicated by a print statement "gyro calibratioon done"
...
/////////////////multi core thread iniitate
  ...
  // th_id++;
  // retval = pthread_create(threads+th_id, NULL, telemetryPID_thread, NULL);
  // if(retval != 0)
  // {
  //   printf("Unable to start thread %d, error code %d\n", th_id, retval);
  //   return retval;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  { 
    ...
    // pid_p_gain_roll = p_roll_tel;               //Gain setting for the roll P-controller
    // pid_i_gain_roll = i_roll_tel;             //Gain setting for the roll I-controller
    // pid_d_gain_roll = d_roll_tel;               //Gain setting for the roll D-controller

    // pid_p_gain_pitch = p_pitch_tel;   //Gain setting for the pitch P-controller.
    // pid_i_gain_pitch = i_pitch_tel;   //Gain setting for the pitch I-controller.
    // pid_d_gain_pitch = d_pitch_tel;   //Gain setting for the pitch D-controller.

    // pid_p_gain_yaw = p_yaw_tel;                   //Gain setting for the pitch P-controller. //4.0
    // pid_i_gain_yaw = i_yaw_tel;               //Gain setting for the pitch I-controller. //0.02
    // pid_d_gain_yaw = d_yaw_tel;    
    ...
    {
      // printf("channel_4:%d \n",channel_4);
      // printf("Flight_mode: %d \t pid_output_altitude: %f\n", flight_mode, pid_output_altitude);
      // printf("pid_altitude_setpoint: %f \n", pid_altitude_setpoint);
      // printf("main loop_timer: %llu\n",get_cpu_usecs() - loop_timer );
      // printf("start: %d takeoff_detected: %d\n",start ,takeoff_detected);
      // printf("esc1:%d esc2:%d esc3:%d esc4:%d\n",esc_1, esc_2, esc_3, esc_4 );
      
      // printf("throttle:%d  pid_output_pitch:%f  pid_output_roll:%f  pid_output_yaw:%f \n",throttle,pid_output_pitch,pid_output_roll,pid_output_yaw);
